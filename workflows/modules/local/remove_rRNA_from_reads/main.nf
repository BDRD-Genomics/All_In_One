...
process Remove_Common_Flora_rRNA_reads {
    ...
    tuple val(sample_id), file("${sample_id}_rRNA_LR.fastq.fastq.gz"), optional: true, emit: rRNA_reads_long
    ...
    """
    if [[ -s "$lr_file" ]]; then
        minimap2 -ax map-ont --secondary=no -t ${task.cpus} ${params.host_db_silva} ${lr_file} > ${sample_id}_host_contaminant_rRNA_removed_LR.sam >> minimap2_removeRNA.log 2>&1
        ...
        samtools fastq -F 4 ${sample_id}_host_contaminant_rRNA_removed_LR.sam | pigz > ${sample_id}_rRNA_LR.fastq.fastq.gz
    fi
    ...
    if [[ -s "$sr_file" ]]; then
        bbmap.sh ${params.bbmap_args} \\
            ...
            -Xmx${java_mem} \\
            outu=${sample_id}_host_contaminant_rRNA_removed_pe_before_repair.fastq.gz overwrite=true >> bbmap_removeRNA.log 2>&1
