...
workflow MMSEQ_AllAssemblers {
  ...
  main:
    ...
    def built_q    = ES_CREATEDB(queries_ch)       // (comp, queryDB_dir/)
    def hits       = ES_SEARCH(built_q, db_for_search)   // (comp, alnDB.dbtype, log)
    def paired     = built_q.join(hits).map { comp, qdir, adbtype, _log ->
                   tuple(comp, qdir, adbtype.parent)  // (comp, queryDB_dir/, alnDB_dir/)
                 }
    def tsvs       = ES_CONVERT(paired, db_for_convert)
