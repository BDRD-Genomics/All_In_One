...
workflow MMSEQ_AllReads {
  ...
  main:
    ...
    // normalize for the consolidated mmseq module
      ...
      tuple(comp, fa, 'queryDB', 'alnDB', 'reads.mmseq.out')
    }
    ...
    //}
    def search_in = built_q.join(search_meta).map { comp, query_dir, aln_prefix, out_suffix ->
	//tuple(comp, query_dir, query_dir, aln_prefix)
	tuple(comp, query_dir)
    }
