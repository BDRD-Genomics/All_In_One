...
opt_list <- list(
  make_option(c("-i", "--input"),
              type = "character",
              help = "Input Excel file from create_nanoplot_qc_stats.R [required]"),
  make_option(c("-s", "--sheet"),
              type = "character",
              default = NULL,
              help = "Worksheet name or index; default: first sheet"),
  make_option(c("-o", "--outdir"),
              type = "character",
              default = ".",
              help = "Output directory"),
  make_option(c("-f", "--outfile"),
              type = "character",
              default = "long_raw_reads_vs_trimmed_reads_w_numbers.jpeg",
              help = "Output JPEG filename with numbers"),
  make_option(c("-r", "--outfile2"),
              type = "character",
              default = "long_raw_reads_vs_trimmed_reads_wo_numbers.jpeg",
              help = "Output JPEG filename without numbers"),
  make_option(c("--width"),
              type = "double",
              default = 12,
...
plot_df <- df %>%
  select(
    Sample,
    Raw = Raw_Num_Reads,
    Post_Distribution = Trimmed_Num_Reads
  ) %>%
  mutate(
    Raw = as.numeric(gsub(",", "", as.character(Raw))),
    Post_Distribution = as.numeric(gsub(",", "", as.character(Post_Distribution)))
  )

plot_long <- plot_df %>%
  pivot_longer(
    cols = c(Raw, Post_Distribution),
    names_to = "Type",
    values_to = "Count"
  ) %>%
  mutate(
    Type = factor(Type, levels = c("Raw", "Post_Distribution")),
...
p <- ggplot(plot_long, aes(x = Sample, y = Count, fill = Type)) +
  ...
  labs(
    title = "Long-read Count: Raw Reads vs. Post-distribution Reads",
    x = "Sample",
    y = "Read Count",
    fill = NULL
  ) +
