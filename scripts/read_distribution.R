...
# -------------------
option_list <- list(
  make_option(c("-i", "--input"), type = "character", help = "Trimmed FASTQ(.gz) file"),
  make_option(c("-p", "--prefix"), type = "character", default = "posttrim", help = "Output file prefix [default = %default]"),
  make_option(c("-o", "--outdir"), type = "character", default = ".", help = "Output directory [default = current directory]")
)

opt <- parse_args(OptionParser(option_list = option_list))

if (is.null(opt$input)) stop("Please provide a --input FASTQ file", call. = FALSE)
...
colnames(q1_df) <- "Q1"
write_csv(q1_df, file.path(opt$outdir, paste0(opt$prefix, "_Q1.csv")))
...
p <- ggplot(read_lengths, aes(x = ReadLength)) +
  geom_histogram(aes(y = ..density..), bins = 100, fill = "#2c7bb6", alpha = 0.15, color = "black") +
  ...
  geom_vline(xintercept = breakpoints, linetype = "dotted", color = "red", linewidth = 0.8) +
